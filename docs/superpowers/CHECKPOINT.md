...
**Stopped at:** Phases 0–9 complete on `main`. Post-migration **Python ↔ Go API parity**: announcements merged (PRs #5/#6); **employees** parity A + B merged to `main` (PRs #7, #9), C (line-manager) in **PR #10** (open, mergeable — merge to finish).
**Branch:** `main` — employees C on `feat/employees-line-manager` (PR #10).
**DB migration version:** **17** on `main` (000017_employee_parity, shipped with PR #7).
...
### Employees — A + B merged to `main`; C in PR #10
19-decision audit (Python `users` module ↔ Go `users`⟂`employees`⟂`dependents`).
Three layered branches; **A + B are on `main`**, C awaits merge:

- **A `feat/employees-parity`** (`a377e44`) — emergency-contact list (new
  ...
  marital/education enums. **Migration 000017.** → **PR #7 MERGED**.
- **B `feat/employees-salary-banking`** (`84c37d6`) — salary/banking field-level
  perms (`users:salary_view/manage`, `users:banking_view/manage`) + account
  masking + write-gate (#6). No migration. → **PR #9 MERGED**.
- **C `feat/employees-line-manager`** (`1db7fd3`+`1b51fc2`) — line-manager suite
  (#10): assignment validation (self / cycle via subordinate-BFS / inactive,
  **advisory-locked in-tx re-check** to close the TOCTOU), `GET /employees/
  manager-candidates`, `GET /employees/{id}/direct-reports`, rich `manager`
  brief `{id,full_name,position,department,is_active}`. No migration. →
  **PR #10 OPEN** (mergeable — merge to complete the employees parity).
...
**Still deferred** → [`handoff-2026-05-29-employee-parity.md`](handoff-2026-05-29-employee-parity.md):
- **#11** cv/id-card upload endpoints (URLs accepted for now).
- **#15** role-level assignment authority (N/A — Go RBAC has no role `level`).
- **Follow-ups found during C** (also worth a small PR): (a) pre-existing
  `GET /employees` uuid list-filters (`department_id`/`position_id`/`manager_id`/
  `role_id`) **400 on any value** — gin can't bind `*uuid.UUID` from a query;
  bind as string + `uuid.Parse` (see `mem:conventions`). (b) employee's **own**
  `department`/`position` names are not resolved on `EmployeeRead` (still null —
  the manager brief resolves them, the self path doesn't; pre-existing Phase-3 gap).

FE: web repo PR **#5** (`feat/go-employees-parity` → main, mergeable) carries the
full FE wiring + `api_info_go/employee.md` + `me.md`. The web repo is self-managed.

**Resume order:** merge PR #10 to finish the BE parity; optionally pick up the
deferred items (#11, #15) + the two follow-ups above. Next migration is **000018**.
