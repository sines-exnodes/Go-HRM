...
> - ⚠️ **Dev DB `exnodes_hrm` is at migration 19, 8 behind disk.** The smoke ran against `exnodes_hrm_test` for that reason. `make migrate-up` needed before any dev deploy.
...
**DB migration version:** latest on disk is **000027** (`password_reset_otps`); next free is **000028**. **Dev `exnodes_hrm` is at 19 and needs `make migrate-up`.** (The "24 / next 25" note below was stale as of 2026-07-16 — 000025 `users_email_partial_unique` and 000026 `employees_insurance_tax` had already landed.)
