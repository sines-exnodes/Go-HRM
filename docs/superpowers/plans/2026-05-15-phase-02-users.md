...
- [ ] **Step 5: List + Get + Patch employee**
...
- [ ] **Step 7: Login as new user + GET /users/me + GET /employees/me**
...
- [ ] **Step 8: Self-update restricted whitelist**
...
- [ ] **Step 9: Self change-password + re-login**
...
- [ ] **Step 10: Dependents CRUD (self-owner)**
...
- [ ] **Step 11: Begin verification log**
...
- [ ] **Step 1: Duplicate email on Create**
...
- [ ] **Step 2: Non-admin tries to LIST employees**
...
- [ ] **Step 3: Non-admin tries to CREATE employee**
...
- [ ] **Step 4: Stranger tries to access another user's dependents**
...
- [ ] **Step 5: Wrong-password admin delete**
...
- [ ] **Step 7: Unauthenticated /employees/me**
...
- [ ] **Step 8: Oversize / wrong-type avatar**
...
- [ ] **Step 9: DB spot check — confirm soft-delete + cascade**
...
- [ ] **Step 10: Append `## Error cases` section to `docs/superpowers/verification/phase-02.md`**
...
- [ ] **Step 1: Append a "Phase 2 — Employees + Dependents endpoints" section**
...
- [ ] **Step 2: Run final DoD checklist (no commits — verification only)**
...
- [ ] Migration `000003_phase2_extras` applied; `device_tokens`, `user_notification_settings`, `employee_leave_quotas` exist with 4 audit cols + UUID PKs + soft-delete triggers.
- [ ] All Phase 2 endpoints listed in the README table are reachable from Swagger UI at `/swagger/index.html` and respond per spec.
- [ ] Login response or `GET /users/me` returns the embedded employee summary (`employee.full_name`, `employee.avatar_url`, `employee.department`, `employee.position`) — so FE doesn't need a separate fetch.
- [ ] Admin can: create employee (in tx with user + roles), list with filters, get, patch, soft-delete (cascade-deactivates user), upload avatar, set leave quota.
- [ ] Admin can: PATCH `/users/:id` to toggle `is_active`, soft-delete user with reauth, reset password, assign roles.
- [ ] Self-service can: GET `/employees/me`, PATCH `/employees/me` with only whitelisted fields, upload own avatar (5MB image only).
- [ ] Self-service can: GET `/users/me`, change own password, change own email (with reauth), register/remove device tokens, toggle notification settings.
- [ ] Dependents CRUD nested under `/employees/:employeeID/dependents`; owner-or-admin enforced at handler.
- [ ] curl from a non-admin token against any admin route returns HTTP 403 with `code: "forbidden"` (verified in Task 25).
- [ ] curl with no token against any `/me*` route returns HTTP 401 (verified in Task 25).
- [ ] All service unit tests pass against a real Postgres test DB (Task 19).
- [ ] Verification log committed to `docs/superpowers/verification/phase-02.md` with both happy-path and error-path sections.
- [ ] README updated with the endpoint table + self-update whitelist.
